...
class EMATeacher(nn.Module):
    ...
    def forward(self, target_img):
        ...
        pseudo_label,pseudo_prob = self.ema_model(target_img)


        #ema_softmax = torch.softmax(logits.detach(), dim=1)
        #pseudo_prob, pseudo_label = torch.max(ema_softmax, dim=1) # pseudo_prob 是 最大值，pseudo_label是索引的位置 ==》 也就是分类的具体地值
