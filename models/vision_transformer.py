...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embedding = nn.Parameter(torch.empty(1, 157, hidden_dim).normal_(std=0.02))  # from BERT
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, input: torch.Tensor):
        torch._assert(input.dim() == 3, f"Expected (batch_size, seq_length, hidden_dim) got {input.shape}")
        #print(self.pos_embedding.shape,"self.pos_embedding")
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if conv_stem_configs is not None:
            ...
            self.conv_proj = nn.Conv2d(
                in_channels=5, out_channels=hidden_dim, kernel_size=(4,5), stride=(4,5),padding=(0,2)
            )
    ...
    def _process_input(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.conv_proj(x)
        #print(x.shape,"x after")
        #print("calculatede",n_h,n_w)
...
def vit_b_16_pos(*, weights: Optional[ViT_B_16_Weights] = None, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    """
...
def vit_b_32_pos(*, weights: Optional[ViT_B_32_Weights] = None, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    """
...
def vit_l_16_pos(*, weights: Optional[ViT_L_16_Weights] = None, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    """
...
def vit_l_32_pos(*, weights: Optional[ViT_L_32_Weights] = None, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    """
...
def vit_h_14_pos(*, weights: Optional[ViT_H_14_Weights] = None, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    """
...
# model = vit_b_16_pos(num_classes=3)
# tensor = torch.randn(1,5,16,193)
